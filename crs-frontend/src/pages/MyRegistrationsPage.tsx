...
export function MyRegistrationsPage() {
    ...
    useEffect(() => {
        // Tách hàm async ra khỏi luồng đồng bộ của Effect để tránh cảnh báo ESLint
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    ...
                    {rows.map((row) => (
                        <tr key={row.id} style={{ borderBottom: '1px solid #eee' }}>
                            ...
                            <td>
                                {new Date(row.ngayDangky || (row as unknown as Record<string, string>).ngayDangKy).toLocaleString('vi-VN')}
                            </td>
                            <td>
